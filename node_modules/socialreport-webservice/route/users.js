...
module.exports = function (app) {
    ...
    return [
...

        {
            ...
            path: '/users/account',
            handler: function (req) {
                model.users.getAccount(req.query.client, req.query.account, function (err, account) {
                    if (err) {
                        return req.reply().code(500);
                    } else {
                        req.reply(account).code(200);
                    }
                });
            }
        },
        // eg. http://127.0.0.1:8000/users/distinct/accounts
        {
            method: 'GET',
            path: '/users/distinct/accounts',
            handler: function (req) {
                model.users.getDistinctAccounts(function (err, account) {
                    if (err) {
                        return req.reply().code(500);
                    } else {
                        req.reply(account).code(200);
